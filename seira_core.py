...
def _default_memory() -> Dict[str, Any]:
    return {
        ...
        "interests": {"warfare": 0, "astrophysics": 0, "lore": 0},
        "notes": [],
        "preferences": {}
    }
...
def load_memory() -> Dict[str, Any]:
    ...
    try:
        with open(MEMORY_PATH, "r", encoding="utf-8") as f:
            ...
            if "interests" not in data: data["interests"] = base["interests"]
...
def get_scheduled_lesson(topic: str, memory: Dict[str, Any]) -> str:
    ...
    if topic == "warfare":
        ...
        user = f"Interest Score: {interest_level}. Avoid: {avoidance_str}. Explain with professional precision."
    else: # Lore
        system = f"You are {AGENT_NAME}. Provide a tactical briefing on Dune universe lore/history."
        user = f"Avoid: {avoidance_str}. Professional scholarly tone."
